...
static void PrintFunction(Proto const *f, int full);
...
static char const *output = Output;     /* actual output file name */
static char const *progname = PROGNAME; /* actual program name */
static TString **tmname;

static void fatal(char const *message)
{
	fprintf(stderr, "%s: %s\n", progname, message);
	exit(EXIT_FAILURE);
}

static void cannot(char const *what)
{
...
static int writer(lua_State *L, void const *p, size_t size, void *u)
{
	UNUSED(L);
...
static int pmain(lua_State *L)
{
	...
	for (i = 0; i < argc; i++)
	{
		char const *filename = IS("-") ? NULL : argv[i];
...
static void PrintType(Proto const *f, int i)
{
	TValue const *o = &f->k[i];
	switch (ttypetag(o))
	{
	case LUA_VNIL:
		printf("N");
...
static void PrintConstant(Proto const *f, int i)
{
	TValue const *o = &f->k[i];
	switch (ttypetag(o))
	{
...
{
	char const *s = f->source ? getstr(f->source) : "=?";
	if (*s == '@' || *s == '=')
		s++;
...
static void PrintDebug(Proto const *f)
{
	int i, n;
	n = f->sizek;
